...
## 기능 목록
 - 만들 다리의 길이를 숫자로 입력받는다.
 - 다리 길이에 맞게 위 또는 아래를 랜덤하게 정답으로 가지는 다리를 만든다.
 - 정답이 위 칸이면 U, 아래 칸이면 D로 나타내어 다리를 문장으로 만든다.
 - 사용자에게 어느 쪽 칸으로 이동할 것인지 입력받는다.
 - 위 칸은 U, 아래 칸은 D로 입력받는다.
 - 이동할 칸이 정답이라면 O를, 건널 수 없다면 X로 표시한다.
 - 이 때 다리 건너기를 실패할 경우 개임을 재시작하거나 종료할 수 있다.
 - 다리를 끝까지 건너면 게임이 종료된다.
 - 재시작 하더라도 처음 생성한 다리를 사용한다.
 - 첫 시도부터 게임을 종료할 때 까지의 시도 횟수를 저장한다. 이후 게임 종료시 총 시도한 횟수를 반환한다.
 - 사용자가 잘못된 값을 입력할 경우 IllegalArgumentException을 발생시키고 [ERROR]메세지를 출력한다.