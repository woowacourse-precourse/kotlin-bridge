...
- [x] BridgeMaker 클래스를 이용해 다리를 생성한다.
    - [x] BridgeNumberGenerator를 이용해서 0과 1중 무작위 값을 생성한다.
    - [x] 값에 따라서 U 또는 D로 나타낸다.
        - 0: 아래 칸이 건널 수 있음(D 저장)
        - 1: 위 칸이 건널 수 있음(U 저장)
...
- [x] 다리를 건너다 실패하면 사용자의 입력에 따라 게임을 재시작하거나 종료한다.
    - R 입력 : 재시작, 처음에 만든 다리로 재사용한다.
    - Q 입력 : 게임 종료
- [x] 게임 결과 총 시도 횟수를 출력한다.
