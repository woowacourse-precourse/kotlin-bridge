...
### 1. 게임 첫 시작 시, “다리 건너기 게임을 시작합니다”를 출력한다.

### 2. 다리의 길이를 입력값으로 받는다.

- 3이상 20이하 정수만 입력받는다
- 예외사항 발생시 ERROR 메시지 출력
  - 예외1) 입력값이 2이하 21이상 정수인 경우
  - 예외2) 입력값에 문자가 포함된 경우
  - 조건에 맞는 입력값을 받을 때 까지 사용자에게 입력값을 요구한다
...
- 예외사항 발생시 ERROR 메시지 출력
  - 예외1) 입력값에 숫자가 포함된 경우
  - 예외2) 입력값에 특수문자가 포함된 경우
  - 예외3) 입력값이 소문자 u 또는 소문자 d일 경우
  - 예외4) 입력값이 대문자 U 또는 대문자  D가 아닌 다른 문자일 경우
  - 조건에 맞는 입력값을 받을 때 까지 사용자에게 입력값을 요구한다

### 5. 사용자가 선택한 칸이 이동할 수 있는 칸인지 확인한다.

**a. 이동할 수 있는 칸이면**
  - 이동한 칸에 O를 출력한다.
  - 선택하지 않은 다른 칸은 공백을 출력한다
  - 사용자에게 이동할 칸을 입력받는다(기능목록 4번으로 되돌아간다)

**b. 이동할 수 없는 칸이면**
  - 이동한 칸에 X를 출력한다.
  - 선택하지 않은 다른 칸은 공백을 출력한다.
  - 사용자에게 재시도하고 싶으면 R , 종료하고 싶으면 Q를 입력 받는다.
    - 예외사항 발생시 ERROR 메시지 출력
      - 예외1) 입력값에 숫자가 포함된 경우
      - 예외2) 입력값에 특수문자가 포함된 경우
      - 예외3) 입력값이 소문자 r 또는 소문자 q일 경우
      - 예외4) 입력값이 대문자 R 또는 대문자  Q가 아닌 다른 문자일 경우
  - 게임 재시도시, 직전에 게임을 어디까지 진행했는지와는 상관없이,기능목록 4번으로 돌아가서 다리 건너기를 처음부터 재시도한다.
  - 게임 종료시, 다리 건너기 최종 결과를 출력하고 성공여부와 시도횟수를 출력한다.

### 6. 다리 건너기를 모두 성공하면 다리 건너기 최종 결과를 출력하고 성공여부와 시도횟수를 출력한다.