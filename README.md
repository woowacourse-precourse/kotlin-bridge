...
1. 게임 시작 문구와 함께 다리의 길이를 입력한다.
  - 입력값의 범위는 3이상 20이하이며 , 예외 처리를 해준다.
  - 에러 메시지를 출력 후 그 부분부터 다시 입력을 받는다.

2. 정상적인 길이가 입력되면, 건널 수 있는 칸을 0과 1로 무작위 값을 이용해 다리를 생성한다.
  - 다리의 길이만큼 각 칸이 0 or 1로 이루어진다.
  - 무작위 값이 0인 경우 아래 칸 , 1인 경우 위 칸이 된다.

3. 다리가 생성되면 이동할 칸을 입력한다.
  - 위 칸은 U , 아래 칸은 D를 입력한다.
  - 입력한 방향으로 이동 가능하면 O가 표시되고 , 불가능하면 X가 표시된다.

4. 이동한 칸이 X(불가능)이면 게임이 종료되고 재시도 여부를 묻는다
  - 재시도와 종료 중 선택한다.
  - 종료하면 이동한 칸까지의 최종 결과가 출력된다.
...
1. (이동할 칸 입력문구)는 다리 크기만큼 나와야 함.
2. 다리 출력 결과는 이동할 칸을 입력한 만큼의 크기
3. 중간에 입력한 결과가 X이면 재시도 여부 문구 출력