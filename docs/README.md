...
### BridgeGame
- [x] 입력값에 따라 칸을 움직인다.
   - [x] 움직이고 난 후, 사용자가 움직인 다리의 결과를 저장한다.
   - [x] 다리 성공 여부를 반환한다.
- [x] 게임을 다시 시작한다.
   - [x] 사용자가 지금까지 움직인 정보들을 초기화 한다.
   - [x] 게임 성공 횟수를 1 증가시킨다.
...
- [ ] 다리 길이 입력값이 올바른지 확인한다.
   - [ ] 입력값에 정수가 아닌 값이 있다면 예외 발생
   - [ ] 다리길이가 3이상 20이하가 아니라면 예외 발생
- [ ] 플레이어 이동할 칸의 입력값이 올바른지 확인한다.
   - [ ] 입력값의 길이가 1이 아니라면 예외 발생
   - [ ] 입력값이 "U" 또는 "D" 가 아니라면 예외 발생
- [ ] 게임 재시작/종료 여부 입력값이 올바른지 확인한다.
   - [ ] 입력값의 길이가 1이 아니라면 예외 발생
   - [ ] 입력값이 "R" 또는 "Q" 가 아니라면 예외 발생
